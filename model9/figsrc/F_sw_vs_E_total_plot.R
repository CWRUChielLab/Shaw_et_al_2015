...
for (i in 1:length(datalist)) {
    ...
    E_per_length = rbind(E_per_length, -((datalist[[i]]$cycle_delta_E_I3 + datalist[[i]]$cycle_delta_E_I2) / datalist[[i]]$cycle_delta_x_sw))
...
#par(mar=c(4,4,4,4))
plot(F_sw[1,], E_per_length[1,], col=datacolors[[1]], type="l",
     ylim=c(0,max(E_per_length[1,])),
     xlab="Force on seaweed",
     ylab="Work done by muscles per unit length of seaweed");
for (i in 2:length(datalist)) {
    lines(F_sw[i,][successful[i,]], E_per_length[i,][successful[i,]], col=datacolors[[i]], type="l", lty=1);
}
dev.off()
